...
public class SetSkinCommand {
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(literal(AmbleKit.MOD_ID)
				...
				.then(literal("skin").then(argument("target", EntityArgumentType.player())
						.executes(SetSkinCommand::executeClear).then(argument("value", StringArgumentType.greedyString()).executes(SetSkinCommand::execute)))));
	}
	...
	private static int execute(CommandContext<ServerCommandSource> context) {
		...
		SkinData data = isUrl ? SkinData.url(value) : SkinData.username(value);
