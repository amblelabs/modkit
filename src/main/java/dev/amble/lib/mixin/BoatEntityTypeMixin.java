...
public class BoatEntityTypeMixin implements BoatTypeRegistry.CustomBoatTypes {
    ...
    @Invoker("<init>")
    public static BoatEntity.Type amblekit$init(String internalName, int ordinal, Block baseBlock, String name) {
        throw new AssertionError();
    }

    @Unique
    private static BoatEntity.Type amblekit$init(BoatTypeRegistry.Entry entry, int offset) {
        return amblekit$init(entry.getEnumName(),
    ...
    public void amble$recalc(List<BoatTypeRegistry.Entry> entries, BoatTypeRegistry.BoatItemConsumer consumer) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            BoatEntity.Type type = variants[l + i] = amblekit$init(entry, i);
