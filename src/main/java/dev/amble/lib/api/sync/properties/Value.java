...
public class Value<T> implements Disposable {
    ...
    public void of(KeyedSyncComponent holder, Property<T> property) {
        this.holder = holder;
    ...
    public SyncComponent getHolder() {
        return holder;
    }
    ...
    public Value<T> with(ServerSyncManager manager) {
    ...
    protected void sync() {
        ...
        if (this.manager == null) {
            throw new NotImplementedException("Manager is not set! Make sure to call with() method before calling sync()");
        }

        if (this.holder == null || !(this.holder.parent() instanceof ServerRootComponent)) {
            return;
        }

        this.manager.markPropertyDirty(this.holder.parent(), this);
    }
    /*
